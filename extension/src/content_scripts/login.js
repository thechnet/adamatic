...
function injectAlert(kind, text) {
	...
	if ($title)
		...
		console.warn('ADAMatic: Cannot find title, not injecting alert.');
}
...
function clickElementById(id) {
	...
	if (!$element)
		alert(`ADAMatic: Cannot find #${id}. Try reloading the page; if the issue persists, please contact the developer.`);
	else
		$element.click();
}
...
chrome.storage.sync.get(['username', 'password']).then(storage => {
	...
	if (document.querySelectorAll('.text-danger,.is-invalid').length > 0) {
		console.info('ADAMatic: Problem detected, aborting.')
		if (storage.password)
	...
	console.trace('ADAMatic: Cannot find #username.');
});
